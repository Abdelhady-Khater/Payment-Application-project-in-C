...
EN_serverError_t saveTransaction(ST_cardData_t  *cardData , ST_terminalData_t *terminalData)
{   
  { time_t Time = time (NULL);
    ...
    if (ptr!=NULL)
    {  
        fprintf(ptr, "satuts:%s\nname:%s\nPAN:%s\nDate:%d/%d/%d\namount:%.2f\n ",cardData->,cardData->cardHolderName,cardData->primaryAccountNumber,date->tm_mday, date->tm_mon+1, date->tm_year+1900,terminalData->transAmount);
        fclose(ptr);
        return SERVER_OK;
    } 
    else
...
void listSavedTransactions (void)
{
  FILE *ptr;
    ...
    {       printf("the last transaction is :\n");
            do {
            i = fgetc(ptr);
            printf("%c", i);
